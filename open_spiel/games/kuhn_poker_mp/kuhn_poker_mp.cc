...
namespace open_spiel
{
  namespace kuhn_poker_mp
  {
    namespace
    {
      ...
      constexpr int kDefaultCards = kDefaultPlayers + 1;
      constexpr double kAnte = 1;

      // Facts about the game
      const GameType kGameType{
          /*short_name=*/"kuhn_poker_mp",
          /*long_name=*/"Kuhn Poker MP",
          ...
          {{"players", GameParameter(kDefaultPlayers)}, {"cards", GameParameter(kDefaultCards)}},
    ...
    public:
                       ...
                       Allocator *allocator) const override
      {
        ...
        SPIEL_CHECK_GE(state.num_cards_, state.num_players_);
        const int num_players = state.num_players_;
        const int num_cards = state.num_cards_;
    ...
    KuhnState::KuhnState(std::shared_ptr<const Game> game)
        : State(game),
          ...
          num_cards_(open_spiel::down_cast<const KuhnGame &>(*game).NumCards()),
          card_dealt_(num_cards_, kInvalidPlayer),
    ...
    void KuhnState::DoApplyAction(Action move)
    {
      ...
      if (first_bettor_ == kInvalidPlayer && num_actions == num_players_)
      {
        ...
        int idx = num_cards_ - 1;
        winner_ = card_dealt_[idx--];
        while (winner_ == kInvalidPlayer) winner_ = card_dealt_[idx--];
      }
               ...
               num_actions == num_players_ + first_bettor_)
      {
        ...
        for (int card = num_cards_ - 1; card >= 0; --card)
        {
    ...
    std::vector<std::pair<Action, double>> KuhnState::ChanceOutcomes() const
    {
      ...
      const double p = 1.0 / (num_cards_ - history_.size());
      for (int card = 0; card < card_dealt_.size(); ++card)
      {
        if (card_dealt_[card] == kInvalidPlayer)
    ...
    KuhnGame::KuhnGame(const GameParameters &params)
        : Game(kGameType, params), num_players_(ParameterValue<int>("players")),
          num_cards_(params.find("cards") != params.end() ? ParameterValue<int>("cards") : num_players_ + 1)
    {
      SPIEL_CHECK_GE(num_players_, kGameType.min_num_players);
      SPIEL_CHECK_LE(num_players_, kGameType.max_num_players);
      SPIEL_CHECK_GE(num_cards_, num_players_);
    ...
    std::vector<int> KuhnGame::InformationStateTensorShape() const
    {
      ...
      // One-hot encoding for the single private card.
      ...
      // n + num_cards_ + 2 (n-1 + n) = 5n - 2 + num_cards_.
      return {5 * num_players_  - 2 + num_cards_};
    }
    ...
    std::vector<int> KuhnGame::ObservationTensorShape() const
    {
      ...
      // n + num_cards_ + n = 2n + num_cards_.
      return {2 * num_players_ + num_cards_};
    }
