...
void BasicKuhnTests() {
  ...
  for (int cards = 3; cards <= 5; cards++) {
    testing::RandomSimTest(
        *LoadGame("kuhn_poker_mp", {{"cards", GameParameter(cards)}}), 100);
  }
  auto observer = LoadGame("kuhn_poker_mp")
                      ->MakeObserver(kDefaultObsType,
                                     GameParametersFromString("single_tensor"));
  testing::RandomSimTestCustomObserver(*LoadGame("kuhn_poker_mp"), observer);
}
...
int main(int argc, char **argv) {
  ...
  open_spiel::testing::CheckChanceOutcomes(*open_spiel::LoadGame(
      "kuhn_poker_mp", {{"players", open_spiel::GameParameter(3)}, {"cards",
                        open_spiel::GameParameter(4)}}));
